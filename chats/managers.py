...
class ChatRoomManager(BaseManager):
    ...
    def join_owners(self, *args, **kwargs):
        return (
            self.get_objects_with_filter(**kwargs)
            .select_related("first_user")
            .select_related("second_user")
            .only(*[f"first_user__{arg}" for arg in args],
                  *[f"second_user__{arg}" for arg in args])
        )
...
class MessagesManager(BaseManager):
    ...
    def join_owners(self, *args, **kwargs):
        return (
            self.get_objects_with_filter(*args, **kwargs)
            .order_by("-id")
